...
*/
export function drawSquarePic(ctx, x, y, w, h, r, url,color='#fff') {
  ctx.save()
  ctx.beginPath()
  // 绘制左上角圆弧
 ...
 */
export function drawTextReturnH(ctx, text, x, y, maxWidth,fontSize,color,textAlign,fontWeight) {
  console.log("drawTextReturnH -> maxWidth", maxWidth)
  ...
  for (let n = 0; n < arrText.length; n++) {
    ...
    ctx.font = fontWeight +' ' + fontSize+'px sans-serif' //设置字体大小，注意：百度小程序 用ctx.setFontSize设置字体大小后，计算字体宽度会无效
    ctx.setFillStyle(color) //设置字体颜色  
    let metrics = ctx.measureText(testLine);//measureText() 方法返回包含一个对象，该对象包含以像素计的指定字体宽度。
    let testWidth = metrics.width;
    if(lineHeight===''){
      lineHeight = testWidth
    }
    if (testWidth > maxWidth && n > 0) {
      ctx.fillText(line, x, y);
      line = arrText[n];
      console.log("drawTextReturnH -> y", y)
      console.log("drawTextReturnH -> lineHeight", lineHeight)
      y += lineHeight;
    } else {
      line = testLine;
    }
